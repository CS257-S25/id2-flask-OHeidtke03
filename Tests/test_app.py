...
class TestFlaskApp(unittest.TestCase):
    ...
    def test_invalid_date_format(self):
        """Test invalid date format that should result in an error message."""
        response = self.app.get('/stats/USA/invalid-date/2022-01-08')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Error: Invalid input or missing data.', response.data)
    def test_invalid_country(self):
        """Test invalid country that should result in an error message."""
        response = self.app.get('/stats/XYZ/2022-01-01/2022-01-08')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Error: Invalid input or missing data.', response.data)
